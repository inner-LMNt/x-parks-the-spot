...
def get_dashboard_analytics(user_id: uuid.UUID, time_filter: str = '30_days', spot_id: Optional[uuid.UUID] = None) -> Result[Dict[str, Any], str]:
    time_deltas = {
        ...
        with conn.cursor(row_factory=dict_row) as cur:
            ...
            for row in spot_metrics_rows:
                ...
                for hour in set(hours):
                    popular_hours.append({'hour': int(hour), 'bookings': hours.count(hour)})
                popular_days = []
                days = row.get('popular_days') or []
                for day in set(days):
                    popular_days.append({'day': day.strip(), 'bookings': days.count(day)})
            ...
            return Ok({
                "overallMetrics": {
                    "revenue": {
                        ...
                        "trends": [{"date": item.get('month', ''), "revenue": item.get('revenue', 0)} for item in monthly_revenue]
                    },
                    "occupancy": {
                        "overallRate": overall_occupancy_rate,
                        "popularTimes": [{"hour": item.get('hour', 0), "bookings": item.get('bookings', 0)} for item in hourly_revenue]
                    },
                    "bookings": {
                ...
                "revenueMetrics": {
                    "monthlyRevenue": [{"month": item.get('month', ''), "revenue": item.get('revenue', 0), "bookings": item.get('bookings', 0)} for item in monthly_revenue],
                    "dailyRevenue": [{"date": item.get('date', ''), "revenue": item.get('revenue', 0)} for item in daily_revenue],
                    "hourlyRevenue": [{"hour": item.get('hour', 0), "revenue": item.get('revenue', 0)} for item in hourly_revenue],
