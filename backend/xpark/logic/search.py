        ...
        with conn.cursor() as cur:
            try:
                ...
                if has_review_filters:
                    ...
                    join_clauses.append("LEFT JOIN reviews r ON parking_spaces.id = r.parking_space_id")
                ...
                has_availability_filters = any(key in filters for key in availability_filters)
                ...
                if has_availability_filters:
                    ...
                    join_clauses.append("LEFT JOIN availability_schedules a ON parking_spaces.id = a.parking_space_id")
                ...
                if has_review_filters:
                    if 'average_rating_min' in filters:
                        having_clauses.append("COALESCE(AVG(r.rating), 0) >= %(average_rating_min)s")
                    if 'average_rating_max' in filters:
                        having_clauses.append("COALESCE(AVG(r.rating), 0) <= %(average_rating_max)s")
                ...
                for row in results:
                    ...
                    if average_rating_present:
                        parking_space["average_rating"] = float(row[average_rating_idx]) if row[average_rating_idx] is not None else None
                ...
                logger.exception("Exception occurred during search_parking_space")
                return Err(str(e))
