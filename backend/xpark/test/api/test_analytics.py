...
def test_analytics_no_and_single_rating(client: FlaskClient) -> None:
    ...
    assert (
        average_ratings["availability"] == 0.0
    ), "Expected average availability rating to be 0.0"
    assert (
        average_ratings["cleanliness"] == 0.0
    ), "Expected average cleanliness rating to be 0.0"
    ...
    assert (
        spot_ratings["ratingDistribution"] == []
    ), "Expected ratingDistribution to be empty"
    ...
    rating_payload = {"availability_rating": 5, "cleanliness_rating": 4}
    ...
    assert (
        response.status_code == 200
    ), f"Failed to get analytics data after rating: {response.get_json()}"
    data = response.get_json()
    assert data is not None
    ...
    average_ratings = rating_metrics.get("averageRatings", {})
    assert (
        average_ratings["availability"] == 5.0
    ), "Expected average availability rating to be 5.0"
    assert (
        average_ratings["cleanliness"] == 4.0
    ), "Expected average cleanliness rating to be 4.0"
    ...
    ratings_by_spot = rating_metrics.get("ratingsBySpot", [])
    assert (
        len(ratings_by_spot) == 1
    ), "Expected one spot in ratingsBySpot after one rating"
    spot_ratings = ratings_by_spot[0]
    ...
    assert (
        spot_ratings["ratingDistribution"] == expected_distribution
    ), "Expected correct ratingDistribution"
...
def test_analytics_multiple_ratings_different_users(client: FlaskClient) -> None:
    ...
    total_total = sum(
        (r["availability_rating"] + r["cleanliness_rating"]) / 2 for r in ratings
    )
    ...
    average_ratings = rating_metrics.get("averageRatings", {})
    assert (
        average_ratings["availability"] == expected_avg_availability
    ), f"Expected average availability rating to be {expected_avg_availability}"
    assert (
        average_ratings["cleanliness"] == expected_avg_cleanliness
    ), f"Expected average cleanliness rating to be {expected_avg_cleanliness}"
    assert (
        average_ratings["total"] == expected_avg_total
    ), f"Expected average total rating to be {expected_avg_total}"

    # Check total ratings
    assert (
        rating_metrics["totalRatings"] == num_raters
    ), f"Expected totalRatings to be {num_raters}"
    ...
    spot_ratings = ratings_by_spot[0]
    assert (
        spot_ratings["ratingCount"] == num_raters
    ), f"Expected ratingCount to be {num_raters}"
    ...
    assert len(actual_distribution) == len(
        expected_distribution_with_percentages
    ...
    for expected in expected_distribution_with_percentages:
        matching = next(
            (
                item
                for item in actual_distribution
                if item["stars"] == expected["stars"]
            ),
            None,
        )
        assert (
            matching is not None
        ), f"Stars {expected['stars']} not found in actual distribution"
        assert (
            matching["count"] == expected["count"]
        ), f"Count mismatch for stars {expected['stars']}"
        assert (
            matching["percentage"] == expected["percentage"]
        ), f"Percentage mismatch for stars {expected['stars']}"

    assert (
        len(spot_ratings["recentReviews"]) == 5
    ), "Expected 5 recent reviews as this is the max"
