...
from ..utils.utils import (
    ...
    create_test_reservation, create_test_report, create_test_reservation_at_time, create_test_image
)
...
def test_create_basic_reports(client: FlaskClient) -> None:
    ...
    other_report = create_test_report(
        ...
        token,
        "Other",
        description="General issue with the system"
    )
...
def test_get_user_reports(client: FlaskClient) -> None:
    ...
    assert reports[0]["type"] in ["Other", "Reservation Issue", "Renter Overstay", "Damage Report"]
...
def test_report_permissions(client: FlaskClient) -> None:
    ...
    report = create_test_report(
        ...
        token1,
        "Other",
        description="Test report for permissions"
    )
...
def test_reservation_timestamps(client: FlaskClient) -> None:
    ...
    time_formats = [
        ...
        ((end_time + timedelta(minutes=30)).astimezone(timezone.utc), "Converted UTC time")
    ]
...
def test_get_report_by_id_full_details(client: FlaskClient) -> None:
    ...
    required_fields = [
        "id", "reservation_id", "user_id", "description",
        "type", "status", "created_at", "updated_at",
        "damage_type", "damage_severity", "image_url",
        "owner_name", "parking_space_name", "parking_space_address",
        "start_time", "end_time"
    ]
...
def test_reservation_price_calculations(client: FlaskClient) -> None:
    ...
    space_id = create_test_parking_space(client, token, price=20.0, name='Test Price Space')
