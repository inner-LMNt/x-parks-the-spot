...
>("/v1/parking-spaces", async () => {
  ...
  const userSpaces = spaces.filter(
    (space) => space.owner_id === authenticatedUserId,
  )
  ...
  console.log(
    `MSW: Returning ${userSpaces.length} parking spaces for owner_id ${authenticatedUserId}`,
  )

  // Return the classified spaces
  return HttpResponse.json(
    { paidSpaces, freeSpaces, pendingSpaces },
    { status: 200 },
  )
})
