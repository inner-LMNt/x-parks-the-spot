 ...
 */
export const deleteUserHandler = http.post<never, { userId: string, password: string }>('user/delete', async ({ request }) => {
    ...
    return HttpResponse.json({ message: 'Account status updated to deleted' }, { status: 200 });
});
