...
import { format, subDays, subMonths, parseISO, differenceInHours, isAfter, isBefore, startOfDay } from 'date-fns';
...
export default function OwnerDashboard() {
    ...
    const { paidSpots, pendingSpots, loading, error } = useAppSelector(state => state.owner);
    const { ownerReservations, loading: reservationsLoading, error: reservationsError } =
    ...
    const chartData = useMemo(() => {
        const grouped = filteredReservations.reduce((acc:any, res:Reservation) => {
            ...
            acc[dateKey].avgDuration = acc[dateKey].totalHours / acc[dateKey].bookings;
    ...
    const spotPerformance = useMemo(() => {
        return paidSpots.map((spot: ParkingSpace) => {
            ...
            return {
                ...
                avgRating: spot.avg_total_rating === 'unrated' ? 'Not rated' :
                    Number(spot.avg_total_rating).toFixed(1),
                ratingCount: (spot.ratings_count_availability || 0) + (spot.ratings_count_cleanliness || 0),
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                    <Card>
                        ...
                        <CardContent>
                            ...
                            <p className="text-xs text-muted-foreground">
                                ${(analytics.totalRevenue / filteredReservations.length || 0).toFixed(2)} per booking
                            </p>
                        </CardContent>
                    </Card>
                    ...
                    <Card>
                        ...
                        <CardContent>
                            ...
                            <p className="text-xs text-muted-foreground">
                                {((analytics.activeBookings / analytics.totalBookings) * 100).toFixed(1)}% of total
                            </p>
                        </CardContent>
                    </Card>
                    ...
                    <Card>
                        ...
                        <CardContent>
                            <div className="text-2xl font-bold">{analytics.avgBookingDuration.toFixed(1)}h</div>
                            <p className="text-xs text-muted-foreground">
                                per booking
                            </p>
                        </CardContent>
                            ...
                            >
                                ...
                                <Card>
                                    <CardHeader>
                                        ...
                                        <CardDescription>Key statistics for your parking spots</CardDescription>
                                    </CardHeader>
                                    <CardContent>
                                        <div className="space-y-4">
                                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                                <div>
                                                    ...
                                                    <p className="text-xl font-bold">{analytics.cancelRate.toFixed(1)}%</p>
                                                </div>
                                                <div>
                                                    <p className="text-sm font-medium text-gray-500">Average Rating</p>
                                                    <div className="flex items-center gap-1">
                                                        <p className="text-xl font-bold">{analytics.avgRating.toFixed(1)}</p>
                                                        <Star className="w-4 h-4 text-yellow-400 fill-yellow-400" />
                                                    </div>
                                                </div>
                                                <div>
                                                    <p className="text-sm font-medium text-gray-500">Active Spots</p>
                                                    <p className="text-xl font-bold">{paidSpots.length}</p>
                                                </div>
                                                <div>
                                                    <p className="text-sm font-medium text-gray-500">Pending Verification</p>
                                                    <p className="text-xl font-bold">{pendingSpots.length}</p>
                                                </div>
                                            </div>
                                        </div>
                                    </CardContent>
                            ...
                            >
                                <Card>
                                    <CardHeader>
                                        ...
                                        <CardDescription>Expected revenue from confirmed bookings</CardDescription>
                                    </CardHeader>
                                    <CardContent>
                                                ...
                                                {upcomingEarnings.map((earning: any, index: any) => (
                                                    ...
                                                    >
                                                        <div className="space-y-1">
                                                            ...
                                                            <p className="text-sm text-gray-500">{earning.spotAddress}</p>
                                                            <p className="text-sm text-gray-500">
                                                                {earning.startDate} at {earning.startTime} • {earning.duration}
                                                            </p>
                                                        </div>
                                ...
                                <Card>
                                    <CardHeader>
                                        ...
                                        <CardDescription>Breakdown of earnings per parking spot</CardDescription>
                                    </CardHeader>
                                    <CardContent>
                                        <div className="space-y-4">
                                            {spotPerformance
                                ...
                                <Star className="w-4 h-4" />
                                                    ...
                                                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                                        <div>
                                                            ...
                                                            <p className="text-lg font-bold">${performance?.revenue || 0}</p>
                                                        </div>
                                                        <div>
                                                            <p className="text-sm text-gray-500">Bookings</p>
                                                            <p className="text-lg font-bold">{performance?.bookings || 0}</p>
                                                        </div>
                                                        <div>
                                                            <p className="text-sm text-gray-500">Base Price</p>
                                                            <p className="text-lg font-bold">${spot.pricing_info?.base_price}/hr</p>
                                                        </div>
                                                        <div>
                                                            <p className="text-sm text-gray-500">Occupancy</p>
                                                            <p className="text-lg font-bold">{performance?.occupancyRate.toFixed(1)}%</p>
                                                        </div>
                                                    </div>
                                                </div>
                                            </CardContent>
