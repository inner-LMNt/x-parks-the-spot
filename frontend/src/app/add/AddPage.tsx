  ...
  const confirmSubmission = async () => {
    ...
    try {
      ...
      if (spotType === 'rental') {
        ...
        // Check if timestamp is within 30 seconds
        const timeDiffSecondsRental = calculateTimeDifferenceSeconds();
        console.log('Time difference (seconds):', timeDiffSecondsRental);
        if (timeDiffSecondsRental > 30) { // **Increased from 15 to 30**
          console.log('Timestamp is beyond the allowed time frame:', timeDiffSecondsRental, 'seconds.');
          toast({
            title: 'Timestamp Error',
            description: 'The photo was not captured within the allowed time frame. Please try again.',
            variant: 'destructive',
