...
import {
    MapPin, Calendar, ChevronDown, ChevronUp, Tag, Clock,
    AlertCircle, ShieldX, Settings, User, DollarSign
} from 'lucide-react';
                ...
                <div className="flex justify-between items-start">
                    <div className="space-y-1">
                        ...
                        <div className="flex items-center gap-3 text-sm text-slate-600">
                            ...
                            <span>{safeFormatDate(report.created_at, 'MMM dd, yyyy, hh:mm a')}</span>
                            <Tag className="w-4 h-4" />
                            <span>{report.parking_space_address || "Location not available"}</span>
                        </div>
                    </div>
                    ...
                    >
                        <CardContent className="bg-slate-50 space-y-6 py-2">
                                ...
                                <div className="space-y-2">
                                    ...
                                    {report.type === 'Renter Overstay' && (
                                        <>
                                            ...
                                            <div className="flex items-center gap-2 text-slate-950">
                                                <Clock className="w-4 h-4 text-slate-600" />
                                                <span><strong>Overstay:</strong> {report.overstay_duration} mins</span>
                                                <DollarSign className="w-4 h-4 text-slate-600 ml-4" />
                                                <span><strong>Charge:</strong> ${report.overstay_charge}</span>
                                            </div>
                                        </>
                                    )}
                            ...
                            <div className="bg-white p-2 rounded-lg border border-slate-200">
                                <p className="text-slate-950"><strong>Description:</strong> {report.description}</p>
                            </div>
                            ...
                            <div className="space-y-2">
                                <textarea
                                    ...
                                    onChange={(e) => setResponseText((prev: any) => ({
                                        ...prev,
        ...
        <CardHeader>
            <div className="flex justify-between items-start">
                <div className="space-y-1">
                    ...
                    <div className="flex items-center gap-3 text-sm text-slate-600">
                        <Calendar className="w-4 h-4" />
                        <span>{safeFormatDate(cancellation.created_at, 'MMM dd, yyyy')}</span>
                        <Tag className="w-4 h-4" />
...
const AdminReportsPage = () => {
    ...
    const { conflicts: reports = [], cancellations = [], loading, error } = useAppSelector(state => state.admin);
        ...
        return (
            <div className="min-h-screen bg-gray-100 py-8">
                <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl">
                    <h1 className="text-3xl font-bold mb-4 text-black">Report Management</h1>
                    <div className="space-y-4">
                        {[...visibleReports, ...visibleCancellations].slice(0, MAX_ITEMS).map((report, i) => (
