...
describe.skip("VerificationPage Component", () => {
  ...
  beforeEach(() => {
    ...
    (getAllPendingSpots as jest.Mock).mockReturnValue({
      type: "admin/getAllPendingSpots",
    });
  });
  ...
  it("renders no pending verification spots", () => {
    ...
    expect(
      screen.getByText(/No pending verification spots available/i),
    ).toBeInTheDocument();
  });
  ...
  it("opens and closes the verification modal", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.queryByText(/Verification Photos/i),
  ...
  it("displays an error message when confirmation input is incorrect", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/You must type 'confirm' to proceed/i),
      ).toBeInTheDocument();
    });
  });
});
