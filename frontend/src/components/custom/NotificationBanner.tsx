...
const NotificationBanner = () => {
  ...
  return (
    domLoaded && (
      <div
        className={`fixed top-0 left-0 right-0 bg-blue-500 text-white p-4 text-center z-50 transition-opacity duration-500 ${fadeOut ? "opacity-0" : "opacity-100"}`}
      >
        <p>
          {upcomingReservation &&
            `You have a reservation starting in less than ${notificationTime} minutes!`}
          {upcomingReservation && <br />}
          {endingReservation &&
            `You have a reservation ending in less than ${notificationTime} minutes!`}
          {endingReservation && <br />}
          {endingReservation &&
            extendable &&
            " You might be able to extend this reservation."}
          {endingReservation &&
            !extendable &&
            " This reservation is not extendable."}
        </p>
        <div>
