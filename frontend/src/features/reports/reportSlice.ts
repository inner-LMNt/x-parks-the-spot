...
export const submitReservationIssueReport = createAsyncThunk<
    ...
    FormData,
    { rejectValue: string }
>('reports/submitReservationIssueReport', async (formData, { rejectWithValue }) => {
    try {
        const response = await axios.post('/reports/reservation-issue', formData);
        return response.data;
    } catch (error: any) {
        return rejectWithValue(
            error.response?.data?.message || 'Failed to submit reservation issue report'
        );
    }
});
...
export const submitRenterOverstayReport = createAsyncThunk<
    ...
    FormData,
    { rejectValue: string }
>('reports/submitRenterOverstayReport', async (formData, { rejectWithValue }) => {
    try {
        ...
        return rejectWithValue(
            error.response?.data?.message || 'Failed to submit renter overstay report'
        );
    }
});
