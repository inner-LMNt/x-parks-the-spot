 ...
 * GET /reservations/{id}/max-extension
 */
export const getMaxExtensionTime = createAsyncThunk<
    { maxExtensionTime: string },
    string,
    { rejectValue: string }
>(
    "reservations/getMaxExtensionTime",
    async (reservationId, { rejectWithValue }) => {
        try {
        ...
        builder
            ...
            .addCase(
                ...
                (state: ReservationsState, action) => {
                    state.loading = false;
                    state.error =
                        action.payload || "Failed to fetch reservation details";
                }
            );
